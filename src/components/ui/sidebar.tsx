...
>(
    (
        ...
        const setWidthWithPersistence = React.useCallback((newWidth: string) => {
            setWidth(newWidth)
...
>(
    (
        ...
        const animateIconWidth = collapsible === "icon" && variant === "inset"

        React.useLayoutEffect(() => {
            if (isMobile || !animateIconWidth) return
            const elements = [gapRef.current, panelRef.current].filter(
                (element): element is HTMLDivElement => element !== null
            )
            const target =
                state === "collapsed" ? "var(--sidebar-width-icon)" : "var(--sidebar-width)"
            // Keep an inline starting width so a state change cannot jump ahead
            // of the spring when the data-attribute CSS updates.
            const initialized = elements.every((element) => element.style.width)
            if (!initialized || reduceMotion || isDraggingRail) {
                for (const element of elements) element.style.width = target
                return
            }
            // Resolve CSS lengths before animating: the expanded variable contains
            // clamp(), which must not be interpolated as a string against rem.
            // One numeric spring also keeps the layout gap and panel in lockstep.
            const panel = panelRef.current
            if (!panel) return
            const from = Number.parseFloat(getComputedStyle(panel).width)
            // Measure an empty, out-of-flow probe instead of laying out all the
            // sidebar content at its destination and then immediately shrinking it.
            const probe = document.createElement("div")
            probe.style.cssText = `position:absolute;visibility:hidden;contain:strict;height:0;width:${target}`
            panel.parentElement?.appendChild(probe)
            const to = Number.parseFloat(getComputedStyle(probe).width)
            probe.remove()
            for (const element of elements) element.style.width = `${from}px`
            let animation: ReturnType<typeof animate> | undefined
            // Start after the state-change render so that render work doesn't
            // consume the beginning of the spring and skip its gentle acceleration.
            const frame = requestAnimationFrame(() => {
                animation = animate(from, to, {
                    ...
                    bounce: 0.1,
                    onUpdate: (value) => {
                        for (const element of elements) element.style.width = `${value}px`
                    },
                    onComplete: () => {
                        for (const element of elements) element.style.width = target
                    }
        ...
        }, [animateIconWidth, isMobile, isDraggingRail, reduceMotion, state, width])
                ...
                <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
                    ...
                    >
                        ...
                        <SheetDescription className="sr-only">
                            Chat navigation, folders, and recent threads.
                        </SheetDescription>
            ...
            >
                ...
                <div
                    ...
                    className={cn(
                        ...
                        animateIconWidth && "transition-none",
